...
<script type="text/javascript">
	$(function(){
		...
		$("select[name=ca1]").find("option").each(function(){
			if("${rvo.ca1}" === $(this).text()){
				$(this).attr("selected", "selected");
			}
		});

		$("select[name=ca2]").find("option").each(function(){
			if("${rvo.ca2}" === $(this).text()){
				$(this).attr("selected", "selected");
			}
		});

		$("select[name=ca3]").find("option").each(function(){
			if("${rvo.ca3}" === $(this).text()){
				$(this).attr("selected", "selected");
			}
		});

		$("select[name=ca4]").find("option").each(function(){
			if("${rvo.ca4}" === $(this).text()){
				$(this).attr("selected", "selected");
			}
		});

		$("select[name=recipe_quant]").find("option").each(function(){
			if("${rvo.recipe_quant}" === $(this).text()){
				$(this).attr("selected", "selected");
			}
		});

		$("select[name=recipe_time]").find("option").each(function(){
			if("${rvo.recipe_time}" === $(this).text()){
				$(this).attr("selected", "selected");
			}
		});

		$("select[name=recipe_difficulty]").find("option").each(function(){
			if("${rvo.recipe_difficulty}" === $(this).text()){
				$(this).attr("selected", "selected");
			}
		});
		...
		<form method="post" id="recipe-form" name="recipe-form">
			<div id="write-top">
				<div id="write-top-left"> 
					...
					<div class="write-input"><textarea rows="1" cols="65" name="recipe_title" placeholder="예)소고기 미역국 끓이기">${rvo.recipe_title}</textarea></div>
					<div class="write-label">요리소개</div>
					<div class="write-input"><textarea rows="3" cols="65" name="recipe_introduce" placeholder="이 레시피의 탄생 배경을 적어주세요.">${rvo.recipe_introduce}</textarea> </div>
					<div class="write-label">동영상</div>
					<div class="write-input" id="write-video">
						<textarea rows="3" cols="40" id="video-url" name="recipe_video" placeholder="동영상이 있으면 주소를 입력하세요.(Youtube만 가능) &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 예)http://youtu.be/lA0Bxo3lZmM">${rvo.recipe_video}</textarea>
						<div id="video-pre">
				...
				<div id="write-top-right">
					...
					<div id="write-top-image">
						<div id="write-top-image-com" hidden="">
							<i class="fas fa-camera"></i>
							<p>요리 대표 사진을 등록해주세요</p>
							<input type='hidden' value='${rvo.main_image}' name='main_image'>
						</div>
						<div id="write-top-image-insert">
							<img onclick="insert_click()" src="${rvo.main_image}">
							<i class="fas fa-times" onclick="del_mainimage()"></i>
						</div>
					</div>
				</div>
			</div>
